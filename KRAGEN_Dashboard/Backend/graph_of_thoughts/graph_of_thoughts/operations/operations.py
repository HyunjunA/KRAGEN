...
class Operation(ABC):
    ...
    def execute(
        ...
        print("Print: In Operation.execute...")
        self.logger.debug("In Operation.execute...")
    ...
    def _execute(
        ...
        :param kwargs: Additional parameters for execution.
        """
        self.logger.debug("In Operation._execute... AbstractMethod....")
        pass
...
class Generate(Operation):
    ...
    def _execute(
        ...
        :param kwargs: Additional parameters for execution.
        """
        print("Print: In Generate._execute...")

        self.logger.debug("In Generate._execute...")

        previous_thoughts: List[Thought] = self.get_previous_thoughts()

        print(f"previous thoughts: {previous_thoughts}")

        if len(previous_thoughts) == 0 and len(self.predecessors) > 0:
            print('no previous thoughts but we have predecessors')
            return

        if len(previous_thoughts) == 0:
            # no predecessors, use kwargs as base state
            previous_thoughts = [Thought(state=kwargs)]
            print("Length of previous thoughts is 0, so setting to Thought(state=kwargs)")
            print(f"previous thoughts: {previous_thoughts}")

        for thought in previous_thoughts:
            base_state = thought.state
            print(f"base_state: {base_state}")
            ...
            print(f"prompt generated by the prompter: {prompt}")
